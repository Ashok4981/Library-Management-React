...
const BookIndex = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `https://localhost:44382/api/books?page=${pageInfo.page}&pageSize=${pageInfo.pageSize}`
      );
  ...
  const handleConfirmDelete = () => {
    axios
      .delete(`https://localhost:44382/api/books/${selectedBook.bookId}`)
      .then((response) => {
        setBooks((prevBooks) =>
  ...
  const handleUpdate = (updatedBook) => {
    const url = `https://localhost:44382/api/books/${selectedBook.bookId}`;
    axios
      .put(url, updatedBook)
      .then((response) => {
